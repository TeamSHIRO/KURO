...
EFI_STATUS efi_main(EFI_HANDLE image_handle_p,
  ...
  const EFI_STATUS bootloader_status = kuro_main();
...
EFI_STATUS init() {
  ...
  char kernel_path[KERNEL_PATH_BUFFER_SIZE];
...
void fini(EFI_STATUS exit_status) {
  if (exit_status == EFI_SUCCESS) {
    ...
    char exit_code[EXIT_CODE_BUFFER_SIZE];
...
EFI_STATUS kuro_main(void) {
  ...
  for (UINTN i = 0; i < app.header.e_phnum; ++i) {
    ...
    const UINT64 p_vaddr = (UINT64)phdr->p_vaddr;
    ...
    const UINT64 segment_end = p_vaddr + p_memsz;
