...
EFI_STATUS
efi_main(EFI_HANDLE image_handle_p, EFI_SYSTEM_TABLE *system_table_p)
{
    ...
    void *input_buffer;
    if (system_table->BootServices->AllocatePool(EfiLoaderData, file_size, &input_buffer) != EFI_SUCCESS)
    {
    ...
    CHAR16 *out;
    if (system_table->BootServices->AllocatePool(EfiLoaderData, (char_count + 3) * sizeof(CHAR16), (void **)&out) !=
        EFI_SUCCESS)
    {
        return EFI_OUT_OF_RESOURCES;
    }
